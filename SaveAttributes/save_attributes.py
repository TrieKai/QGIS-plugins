...
class SaveAttributes:
    ...
    def state_changed(self, checked):
        ...
        checkBoxName = self.dlg.sender().text()
        #print self.dlg.sender().text()
        layerCode = dictLayers.get(checkBoxName)
        if checked:
            legend.setLayerVisible(layerCode,True)
        else:
            legend.setLayerVisible(layerCode,False)
        #if legend.layers()[1].selectedLayers:
    ...
    def run(self):
        ...
        for i in range(len(layer_list)):               
            CheckBox = QCheckBox(layer_list[i])
            if self.iface.legendInterface().isLayerVisible(layers[i]) == True:
                CheckBox.setChecked(True)
            else:
                CheckBox.setChecked(False)
            CheckBox_list.append(CheckBox)           
            layout.addRow(CheckBox_list[i])
            
        global dictLayers
        dictLayers = {}
        for a in range(len(layer_list)):
            CheckBox_list[a].stateChanged.connect(self.state_changed)
            dictLayers[layer_list[a]] = layers[a]
